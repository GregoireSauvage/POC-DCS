...
def list_halls(
    ...
    for h in halls:
        spectator_count = (
            db.query(Spectator)
            .filter(Spectator.tenant_id == p.tenant_id, Spectator.hall_id == h.id)
            .count()
        )
...
def create_one(
    ...
    pi = build_policy_input(
        ...
        resource_type="hall",
        resource_id="new",
        owner_id=str(payload.owner_user_id),
        crypto_meta={},
    )
    dec = evaluate(pi)
    dh = decision_hash(dec)
    ...
    return {
        "id": hall.id,
        "name": hall.name,
        "current_film_id": hall.current_film_id,
        "owner_user_id": hall.owner_user_id,
        "spectator_count": 0,
    }
