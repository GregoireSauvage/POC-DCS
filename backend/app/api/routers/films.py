...
def list_films(
    ...
    for film in films:
        ...
        applied = apply_decision(
            decision=dec,
            ciphertext_row=row,
            field_to_ciphertext={"time_elapsed": "time_elapsed_ct"},
        )
        payload = {
            "id": film.id,
            "title": film.title,
            "time_elapsed": applied.payload.get("time_elapsed"),
        }
...
def create_one(
    ...
    film = create_film(
        db, tenant_id=p.tenant_id, title=payload.title, time_elapsed=payload.time_elapsed
    )
    write_audit(
        db=db,
        request_id=request.state.request_id,
    ...
    row = {"title": film.title, "time_elapsed_ct": film.time_elapsed_ct}
    applied = apply_decision(
        decision=dec_r, ciphertext_row=row, field_to_ciphertext={"time_elapsed": "time_elapsed_ct"}
    )
    write_perf(
        db=db,
        request_id=request.state.request_id,
