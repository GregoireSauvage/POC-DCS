...
from app.dcs.pdp.engine import decision_hash, evaluate
...
def create_one(
    request: Request,
    payload: SpectatorCreate,
    db: Annotated[Session, Depends(get_db)],
    p: Annotated[Principal, Depends(get_current_principal)],
):
    hall = (
        db.query(Hall)
        .filter(Hall.tenant_id == p.tenant_id, Hall.id == payload.hall_id)
        .one_or_none()
    )
    if hall is None:
    ...
    pi = build_policy_input(
        ...
        resource_id="new",
        owner_id=str(hall.owner_user_id),
        crypto_meta={
    ...
    row = {
        ...
        "name_ct": sp.name_ct,
        "age_ct": sp.age_ct,
        "external_id_ct": sp.external_id_ct,
    }
    applied = apply_decision(
        decision=dec_r,
        ciphertext_row=row,
        field_to_ciphertext={"name": "name_ct", "age": "age_ct", "external_id": "external_id_ct"},
    )
...
def search_by_external_id(
    ...
    matches = (
        db.query(Spectator)
        .filter(Spectator.tenant_id == p.tenant_id, Spectator.external_id_lookup == lookup)
        .all()
    )
    ...
    for sp in matches:
        ...
        applied = apply_decision(
            ...
            field_to_ciphertext={
                "name": "name_ct",
                "age": "age_ct",
                "external_id": "external_id_ct",
            },
        )
        spectator_id = (
            str(sp.id) if (p.role == "admin" or not dcs_enabled()) else mask_uuid(str(sp.id))
        )
        out.append(
            {
                "id": spectator_id,
